...
def warmup_models(model_players, model_keypoints, device='cuda'):
    """
    Warm up GPU models to ensure optimal performance.
    First inference is always slow, this pre-warms the GPU.
    """
    ...
    with torch.no_grad():
        for _ in range(3):
            model_players(dummy_img, imgsz=480, half=True, verbose=False)
            model_keypoints(dummy_img, imgsz=480, half=True, verbose=False)
...
class VideoReader(threading.Thread):
    """
    Threaded video reader to avoid I/O bottleneck.
    """
    def __init__(self, cap, queue_size=3):
        super().__init__()
        self.cap = cap
...
def get_dominant_colors_fast(img_rgb, n_colors=3):
    """
    Fast color extraction using OpenCV k-means instead of PIL.
    Much faster than PIL palette method.
    """
    # Reshape to pixels
...
def predict_teams_vectorized(obj_palette_list, color_list_lab, nbr_team_colors=2):
    """
    Vectorized team prediction using NumPy operations.
    Much faster than nested loops.
    """
    if len(obj_palette_list) == 0:
        return []
    
    # Convert all palettes to LAB in batch
    all_palettes_lab = []
    for palette in obj_palette_list:
        palette_lab = np.array([skimage.color.rgb2lab([c/255 for c in color]) for color in palette])
        all_palettes_lab.append(palette_lab)
    
    color_list_lab_arr = np.array(color_list_lab)
    
    players_teams_list = []
    for palette_lab in all_palettes_lab:
        # Calculate all distances at once using broadcasting
        # palette_lab: (n_colors, 3), color_list_lab_arr: (n_teams*nbr_team_colors, 3)
        distances = np.linalg.norm(palette_lab[:, np.newaxis, :] - color_list_lab_arr[np.newaxis, :, :], axis=2)
        
        # Get team votes for each color
        min_indices = np.argmin(distances, axis=1)
        team_votes = min_indices // nbr_team_colors
        
        # Majority vote
        team_idx = np.bincount(team_votes).argmax()
        players_teams_list.append(team_idx)
    
    return players_teams_list


def transform_points_homography(points, homog):
    """
    Vectorized homography transformation for multiple points.
    """
    if len(points) == 0:
        return np.array([])
    ...
    # Apply homography
    transformed = (homog @ points_h.T).T
...
def detect(cap, stframe, output_file_name, save_output, model_players, model_keypoints,
    ...
    keypoint_detection_interval = 5  # Only detect keypoints every N frames (more aggressive)
    frame_skip = 1  # Process every Nth frame (set to 2 to double FPS)
    inference_size = 480  # Smaller = faster (default 640)
    last_results_keypoints = None
    last_homog = None
    
    # Warmup GPU for optimal performance
    st.toast("🚀 Warming up GPU...")
    warmup_models(model_players, model_keypoints)
    st.toast("✅ GPU ready!")
    ...
    for frame_nbr in range(1, tot_nbr_frames+1):
        ...
        if success:
            ...
            # Run YOLOv8 players inference on the frame (OPTIMIZED: FP16 + smaller size + CUDA)
            results_players = model_players(frame, conf=p_conf, half=True, verbose=False, 
                                           imgsz=inference_size, device='cuda')
            
            # Run YOLOv8 field keypoints inference only every N frames (OPTIMIZATION)
            if frame_nbr % keypoint_detection_interval == 1 or last_results_keypoints is None:
                results_keypoints = model_keypoints(frame, conf=k_conf, half=True, verbose=False,
                                                   imgsz=inference_size, device='cuda')
                last_results_keypoints = results_keypoints
            else:
                results_keypoints = last_results_keypoints
            ...
            if 'homog' in locals():
                ...
                # OPTIMIZED: Vectorized transformation of player coordinates using homography matrix
                pred_dst_pts = transform_points_homography(detected_ppos_src_pts, homog)
                ...
                if detected_ball_src_pos is not None:
                    ball_transformed = transform_points_homography([detected_ball_src_pos], homog)
                    detected_ball_dst_pos = ball_transformed[0] if len(ball_transformed) > 0 else None
            ...
            ###########################
            # === OPTIMIZED VERSION: Using fast OpenCV k-means and vectorized operations ===

            frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)                                      # Convert frame to RGB
            obj_palette_list = []                                                                   # Initialize players color palette list
            
            # Cache CPU conversions to avoid repeated transfers
            player_labels = results_players[0].boxes.cls.cpu().numpy()
            player_bboxes = results_players[0].boxes.xyxy.cpu().numpy()

            ## Loop over detected players (label 0) and extract dominant colors palette using FAST k-means
            for i, j in enumerate(player_labels):
                if int(j) == 0:
                    bbox = player_bboxes[i,:]                                                       # Get bbox info (x,y,x,y)
                    obj_img = frame_rgb[int(bbox[1]):int(bbox[3]), int(bbox[0]):int(bbox[2])]       # Crop bbox out of the frame
                    
                    if obj_img.size == 0:
                        obj_palette_list.append([[128, 128, 128]] * num_pal_colors)
                        continue
                    ...
                    # OPTIMIZED: Use fast OpenCV k-means instead of PIL
                    palette = get_dominant_colors_fast(center_filter, num_pal_colors)
                    obj_palette_list.append(palette)

            ## OPTIMIZED: Vectorized team prediction
            players_teams_list = predict_teams_vectorized(obj_palette_list, color_list_lab, nbr_team_colors)
