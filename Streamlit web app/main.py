...
def main():
    ...
    # Load the YOLOv8 players detection model with CUDA acceleration
    @st.cache_resource
    def load_models():
        import torch
        ...
        model_p = YOLO("../models/Yolo8L Players/weights/best.pt")
        model_p.to('cuda')
        model_p.fuse()  # Fuse Conv+BN first (requires float32)
        
        model_k = YOLO("../models/Yolo8M Field Keypoints/weights/best.pt")
        model_k.to('cuda')
        model_k.fuse()  # Fuse Conv+BN first (requires float32)
        
        return model_p, model_k
    
    model_players, model_keypoints = load_models()
